...
const reduceState = (state, action) => {
    switch (action.type) {
        ...
        case 'MOVE_CURSOR':
            return {
                ...
                ui: {
                    ...state.ui,
                    cursor: {
                        r: Math.max(0, Math.min(BOARD_SIZE - 1, state.ui.cursor.r + action.dR)),
                        c: Math.max(0, Math.min(BOARD_SIZE - 1, state.ui.cursor.c + action.dC))
                    }
                }
            };
        ...
        case 'INTERACT_CELL':
            ...
            console.log(`Interacted at: (${r}, ${c}) | Phase: ${nextGame.turnPhase}`);
            ...
            if (nextGame.turnPhase === 'PLACE') {
                ...
                if (cell.worker && cell.worker.startsWith(nextGame.currentPlayer)) {
                    nextGame = selectWorker(nextGame, r, c);
                    nextUI.lastWorkerByPlayer[nextGame.currentPlayer] = cell.worker;
                    nextUI.highlightedCells = getValidMoves(nextGame, r, c);
                }
            }
            else if (nextGame.turnPhase === 'MOVE') {
                const cell = nextGame.board[r][c];
                const isOwnWorker = cell.worker && cell.worker.startsWith(nextGame.currentPlayer);
                ...
                if (isOwnWorker && isTheSelectedWorker) {
                    ...
                    const otherPos = findWorkerPosition(nextGame.board, otherWorkerId);
                    if (otherPos) {
                        nextGame = selectWorker(nextGame, otherPos.r, otherPos.c);
                        nextUI.lastWorkerByPlayer[nextGame.currentPlayer] = otherWorkerId;
                        nextUI.highlightedCells = getValidMoves(nextGame, otherPos.r, otherPos.c);
                ...
                if (isClickValid(state.ui.highlightedCells, r, c)) {
                    ...
                    if (nextGame.turnPhase === 'SELECT') {
                        const rememberedWorker = nextUI.lastWorkerByPlayer[nextGame.currentPlayer];
                        const rememberedPos = rememberedWorker ? findWorkerPosition(nextGame.board, rememberedWorker) : null;
                        if (rememberedPos) nextUI.cursor = rememberedPos;
                    }
                }
            }

            // Fire Win Alert (A controlled side-effect outside the pure UI loop)
            if (nextGame.turnPhase === 'GAMEOVER' && state.game.turnPhase !== 'GAMEOVER') {
                setTimeout(() => alert(`GAME OVER! ${nextGame.winner} HAS WON THE GAME!`), 10);
            }
...
const render = (state) => {
    ...
    boardElement.style.setProperty('--cursor-color', PLAYER_CURSOR_COLOR[state.game.currentPlayer]);

    if (statusElement) {
        statusElement.innerText = `Turn: ${state.game.currentPlayer} | Phase: ${state.game.turnPhase}`;
    }
    ...
    state.game.board.forEach((row, r) => {
        row.forEach((cell, c) => {
            ...
            const isCursor = (r === state.ui.cursor.r && c === state.ui.cursor.c);
            cellDiv.tabIndex = isCursor ? 0 : -1;

            // ACTION: Dispatch instead of direct logic
            cellDiv.addEventListener('click', () => dispatch({ type: 'INTERACT_CELL', r, c }));
...
boardElement.addEventListener('keydown', (e) => {
    switch (e.key) {
        case 'ArrowUp':    dispatch({ type: 'MOVE_CURSOR', dR: -1, dC: 0 }); break;
        case 'ArrowDown':  dispatch({ type: 'MOVE_CURSOR', dR: 1, dC: 0 }); break;
        case 'ArrowLeft':  dispatch({ type: 'MOVE_CURSOR', dR: 0, dC: -1 }); break;
        case 'ArrowRight': dispatch({ type: 'MOVE_CURSOR', dR: 0, dC: 1 }); break;
        case ' ':
        case 'Enter':
            e.preventDefault();
            dispatch({ type: 'INTERACT_CELL', r: appState.ui.cursor.r, c: appState.ui.cursor.c });
            return;
        default:
            return; 
    }
